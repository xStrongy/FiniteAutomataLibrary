...
namespace FiniteAutomataLibrary
{
    public class AutomataBuilder
    {
            ...
            string RegularExpression, string Alphabet)
        {
            ...
            {
                NondeterministicFiniteAutomaton NFA2 = EmptySet();
                return NFA2;
            }
        ...
        public NondeterministicFiniteAutomaton BuildAutomatonFromRegularGrammar(List<string> RegularGrammar)
        {
            ...
            foreach (string line in RegularGrammar)
            {
                ...
                foreach (string partOfLine in splittedFromPipes)
                {
                    ...
                    if (!splitFromNonterminals[2].Equals("") && !Nonterminals.Contains(splitFromNonterminals[2].Substring(1, splitFromNonterminals[2].Length - 2)))
                    {
                        Nonterminals.Add(splitFromNonterminals[2].Substring(1, splitFromNonterminals[2].Length - 2));
                        State state = new State(stateCounter + 1, splitFromNonterminals[2].Substring(1, splitFromNonterminals[2].Length - 2), false, false);
                        stateCounter++;
                        states.Add(state);
                    }
                    ...
                    if (splitFromNonterminals[1].Length > 1 && splitFromNonterminals[2].Equals(""))
                    {
                        for (int i = 0; i < splitFromNonterminals[1].Length; i++)
                        {
                            ...
                            {
                                triplets.Add(new DeltaFunctionTriplet(lastId - 1,
                                   splitFromNonterminals[1][i], newState.Id));
                            }
                        }
        ...
        private NondeterministicFiniteAutomaton EmptySet()
        {
            List<State> states = new List<State>();
            List<DeltaFunctionTriplet> triplets = new List<DeltaFunctionTriplet>();
            SortedList<int, List<int>> Epsilons = new SortedList<int, List<int>>();
